...
public class plSqlTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //词法分析器
        PlSqlLexer mySqlLexer = new PlSqlLexer(CharStreams.fromString(sql.toUpperCase()));
        //词法符号的缓冲区,用于存储词法分析器生成的词法符号
        CommonTokenStream commonTokenStream = new CommonTokenStream(mySqlLexer);
        //新建一个语法分析器，处理词法符号缓冲区内容
        PlSqlParser plSqlParser = new PlSqlParser(commonTokenStream);
        //获取出selectStatement
        ParseTree tree = plSqlParser.select_statement();
        System.out.println(printSyntaxTree(plSqlParser,tree));
    }
