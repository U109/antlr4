...
//定义了一个名为DELIMITER的词法标记，用于表示SQL语句中的分隔符。
tokens {
    DELIMITER
}

//定义规则：singleStatement规则表示一个完整的SQL语句。以EOF结尾，表示输入必须是完整的。
singleStatement
    : statement EOF
    ;

//定义规则：standaloneExpression表示一个独立的表达式。以EOF结尾，表示输入必须是完整的。
